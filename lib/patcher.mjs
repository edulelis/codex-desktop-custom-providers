...
export const WHAM_CAPABILITY_TOKEN = MARKER_TEXT + "V4";
...
export function upgradeRequestBundle(filePath) {
  ...
  // A V2 send region is already current; the wham strip is applied separately.
...
export function patchWhamUpsell(assets) {
  ...
  const STRIP_FIELDS =
    ...
    "rate_limit:void 0,rate_limit_reset_credits:void 0";
  ...
  for (const filePath of walkAssets(assets)) {
    ...
    const buildStrip = (resp, client, url, brand) =>
      ...
      `try{${resp}={...${resp},${STRIP_FIELDS}}}catch{};` +
      "let ";
    ...
    if (oldMatch !== null) {
      ...
      const patched = source.replace(oldStripRe, (_match, respVar) => {
        return `/*${WHAM_CAPABILITY_TOKEN}*/try{${respVar}={...${respVar},${STRIP_FIELDS}}}catch{}`;
      });
...
export function reportWham(result) {
  if (result === "applied") status("OK", "Usage upsell banner suppression (V4) applied.");
