...
export const INJECTION_VERSION = "V4";
...
export const SEND_INJECT_TEMPLATE =
  ...
  "if(e===`thread/fork`&&t!=null&&typeof t===`object`&&t.model!=null&&t.modelProvider==null)try{" +
  ...
  "}catch{}" +
  // Remember the provider the user picked (settings/update) so the next
  // thread/resume can rebind it: the core rebuilds an idle, unsubscribed
  // thread from resume overrides (model_provider), which is the sanctioned
  // way to move a conversation across providers without forking.
  ...
  "typeof P===`string`&&P.length>0&&((this.__codexProvStash??={})[t.threadId]=P)" +
  "}catch{}" +
