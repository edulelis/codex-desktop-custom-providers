...
export const INJECTION_VERSION = "V3";
...
export function upgradeRequestBundle(filePath) {
  ...
  // Drop the legacy (V2) settings/update injection: the protocol has no
  // modelProvider field there, so it was a silent no-op server-side.
  const legacySettingsRe = new RegExp(
    "if\\(\\(e===`thread/settings/update`\\|\\|e===`turn/settings/update`\\)[\\s\\S]*?\\}catch\\{\\}", "s"
  );
  source = source.replace(legacySettingsRe, "");
  ...
  if (countOccurrences(source, ROUTING_FILENAME) !== 3 || (markerCount !== 2 && markerCount !== 3)) {
