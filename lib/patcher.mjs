...
export function upgradeRequestBundle(filePath) {
  ...
  if (source.includes(CAPABILITY_TOKEN)) {
    throw new PatchError(`${path.basename(filePath)} already contains the V2 injection.`);
  }
  const ipcHelper = resolveIpcHelper(source);
  ...
  try {
    ...
    if (patchedBundle !== null) {
      ...
      if (previous.includes(CAPABILITY_TOKEN)) {
        status("OK",
          "Request-layer provider routing (with mid-thread provider swaps) is already installed.");
        return null;
      }
      status("OK", `Found existing provider-routing patch: ${path.basename(patchedBundle)}`);
      if (dryRun) {
