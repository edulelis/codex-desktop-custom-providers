...
//   - thread/fork (V3): forking a conversation to a picked model resolves the
//     provider from the routing file — the sanctioned way to move a
//     conversation across providers mid-course (settings/update has no
//     modelProvider field server-side).
//
...
// NOTE: kept distinct from the wham strip token suffix to avoid
// capability-token collisions between injection sites.
export const INJECTION_VERSION = "V8";
...
export const WHAM_CAPABILITY_TOKEN = MARKER_TEXT + "V5";
...
export const SEND_INJECT_TEMPLATE =
  "/*" + CAPABILITY_TOKEN + "*/" +
  // Live channel: config + diagnostics. Reads
  // ~/.codex/provider-routing-live.json (30s cache, fail-open) for feature
  // flags, and when debug is on appends JSONL events to
  // ~/.codex/provider-routing-events.log — both editable without re-rooting.
  "if(globalThis.__codexLiveCfg==null||Date.now()-globalThis.__codexLiveCfgT>30000)try{" +
  "let{codexHome:__ch}=await @IPC@(`codex-home`,{params:{hostId:this.hostId}})," +
  "{contents:__lc}=await @IPC@(`read-file`,{params:{hostId:this.hostId,path:__ch+`/provider-routing-live.json`}});" +
  "globalThis.__codexLiveCfg=JSON.parse(__lc);globalThis.__codexLiveCfgT=Date.now()" +
  "}catch(__e){globalThis.__codexLiveCfg=globalThis.__codexLiveCfg||{};globalThis.__codexLiveCfgT=Date.now()}" +
  "let _c=globalThis.__codexLiveCfg||{};" +
  "if(globalThis.__codexDiag==null)globalThis.__codexDiag=async(__ev,__d)=>{" +
  "try{let _c=globalThis.__codexLiveCfg||{};if(!_c.debug)return;" +
  "let{codexHome:__ch}=await @IPC@(`codex-home`,{params:{hostId:this.hostId}})," +
  "__lp=__ch+`/provider-routing-events.log`," +
  "__cur=await @IPC@(`read-file`,{params:{hostId:this.hostId,path:__lp}}).catch(()=>({contents:``}))," +
  "__lines=(__cur.contents+`\\n`+JSON.stringify({ts:new Date().toISOString(),ev:__ev,...(__d||{})})).split(`\\n`).slice(-2000).join(`\\n`)," +
  "__b64=btoa(unescape(encodeURIComponent(__lines)));" +
  "await @IPC@(`fs/writeFile`,{params:{hostId:this.hostId,path:__lp,dataBase64:__b64}})}catch{}};" +
  "await globalThis.__codexDiag(`req`,{m:e,model:t&&t.model,thread:t&&t.threadId});" +
  "e===`thread/list`&&(t==null||typeof t!==`object`?t={modelProviders:[]}:" +
  "t.modelProviders==null&&(t={...t,modelProviders:[]}));" +
  "if((_c.features||{}).startRouting!==false&&e===`thread/start`&&t!=null&&typeof t===`object`&&t.modelProvider==null)try{" +
  ...
  "globalThis.__codexRoutingLast=s;" +
  "typeof c===`string`&&c.length>0&&(t={...t,modelProvider:c})" +
  "}catch{}" +
  // thread/fork accepts model + modelProvider (fork = new thread with
  // history), so forking after picking a model is the sanctioned way to move
  // a conversation across providers. settings/update has NO modelProvider
  // field server-side; the provider binds at thread/start.
  "if((_c.features||{}).forkRouting!==false&&e===`thread/fork`&&t!=null&&typeof t===`object`&&t.threadId!=null)try{" +
  "if(t.model==null&&this.__codexProvModelStash!=null&&this.__codexProvModelStash[t.threadId]!=null)" +
  "t={...t,model:this.__codexProvModelStash[t.threadId]};" +
  "if(t.model!=null&&t.modelProvider==null){" +
  "let{codexHome:j}=await @IPC@(`codex-home`,{params:{hostId:this.hostId}})," +
  "w=j.includes(`\\\\`)&&!j.includes(`/`)?`\\\\`:`/`," +
  "x=`${j.replace(/[\\\\/]+$/u,``)}${w}" + ROUTING_FILENAME + "`," +
  "{contents:y}=await @IPC@(`read-file`,{params:{hostId:this.hostId,path:x}})," +
  "z=JSON.parse(y),A=z?.model_providers?.[t.model]??z?.default_provider;" +
  "globalThis.__codexRoutingLast=z;" +
  "typeof A===`string`&&A.length>0&&(t={...t,modelProvider:A})}" +
  "}catch{}" +
  // Remember the model+provider the user picked (settings/update). The
  // provider feeds the thread/resume rebind (cold-resume rebuilds); the
  // model feeds thread/fork inheritance, so the native Fork action carries
  // the picked model (and its routed provider) into the new thread — the
  // sanctioned cross-provider move with native UI navigation.
  "if((_c.features||{}).stash!==false&&e===`thread/settings/update`&&t!=null&&typeof t===`object`&&t.model!=null&&t.threadId!=null)try{" +
  "let{codexHome:K}=await @IPC@(`codex-home`,{params:{hostId:this.hostId}})," +
  "L=K.includes(`\\\\`)&&!K.includes(`/`)?`\\\\`:`/`," +
  "M=`${K.replace(/[\\\\/]+$/u,``)}${L}" + ROUTING_FILENAME + "`," +
  "{contents:N}=await @IPC@(`read-file`,{params:{hostId:this.hostId,path:M}})," +
  "O=JSON.parse(N),P=O?.model_providers?.[t.model]??O?.default_provider;" +
  "globalThis.__codexRoutingLast=O;" +
  "typeof P===`string`&&P.length>0&&((this.__codexProvStash??={})[t.threadId]=P,(this.__codexProvModelStash??={})[t.threadId]=t.model)" +
  "}catch{}" +
  "if((_c.features||{}).resumeRebind!==false&&e===`thread/resume`&&t!=null&&typeof t===`object`&&t.threadId!=null" +
  "&&this.__codexProvStash!=null&&this.__codexProvStash[t.threadId]!=null)try{" +
  "t={...t,modelProvider:this.__codexProvStash[t.threadId]};delete this.__codexProvStash[t.threadId]" +
  "}catch{}";
...
// Picker redirect (V7): when a follow-up message carries a model whose
// provider differs from the conversation's bound provider, redirect the pick
// into the app's native fork flow — the fork inherits the picked model and
// provider, and the app itself migrates the open view to the new thread, so
// the switch feels in-place. Same-provider picks behave exactly as before.
export const PICKER_RE = new RegExp(
  "await (\\w+)\\.updateThreadSettingsForNextTurn\\((\\w+),\\{model:(\\w+)," +
  "effort:(\\w+)===void 0\\?(\\w+)\\?\\.latestReasoningEffort\\?\\?null:(\\w+)\\}\\)"
, "g");
export const PICKER_MARKER = MARKER_TEXT + "PickerFork";
export function buildPickerInjection(manager, threadId, model, effort, conv, effortFallback) {
  const orig =
    `await ${manager}.updateThreadSettingsForNextTurn(${threadId},` +
    `{model:${model},effort:${effort}===void 0?${conv}?.latestReasoningEffort??null:${effortFallback}}})`;
  return (
    `/*${PICKER_MARKER}*/await(async()=>{try{let _C=globalThis.__codexLiveCfg||{};if((_C.features||{}).pickerRedirect!==true)return;` +
    `let _R=globalThis.__codexRoutingLast,` +
    `_P=_R?.model_providers?.[${model}]??_R?.default_provider,` +
    `_B=${manager}.getConversation(${threadId})?.modelProvider;` +
    `if(_P!=null&&_B!=null&&_P!==_B){` +
    `let _y=null;` +
    `await ${manager}.forkConversationFromLatest({sourceConversationId:${threadId},model:${model},afterConversationCreated:(_z)=>{_y=_z}});` +
    `if(_y!=null){${threadId}=_y}else{${orig}}` +
    `}else{${orig}}` +
    `}catch(_pkErr){${orig}}})()`
  );
}
...
export function ensureLiveConfig(configPath) {
  const livePath = path.join(path.dirname(configPath), "provider-routing-live.json");
  if (!fs.existsSync(livePath) || fs.statSync(livePath).size === 0) {
    // pickerRedirect stays OFF until diagnosed (V7 broke app load); debug ON
    // so the events log captures every routing decision from the start.
    const defaults = { debug: true, features: { pickerRedirect: false } };
    fs.writeFileSync(livePath, JSON.stringify(defaults, null, 2) + "\n");
    status("CONFIG", "Live-config channel created.", livePath);
  }
}

function validateNoGlobalProvider(configPath) {
...
export function patchRequestBundle(filePath) {
  ...
  if (countOccurrences(patched, ROUTING_FILENAME) !== 4) {
...
export function upgradeRequestBundle(filePath) {
  ...
  if (!source.includes(CAPABILITY_TOKEN)) {
    // Older injection: swap the whole sendRequest injection region (marker
    // comment up to the statement's `return e===`config/read`` tail) for the
    // current template. Covers any previous version's block layout.
    const ipcHelper = resolveIpcHelper(source);
    const newInjection = SEND_INJECT_TEMPLATE.replaceAll("@IPC@", ipcHelper);
    const regionRe = new RegExp(
      escapeRegExp("/*" + MARKER_TEXT) + "\\w*\\*/[\\s\\S]*?(?=return e===`config/read`)", "s"
    );
    let regions = 0;
    source = source.replace(regionRe, () => {
      regions += 1;
      return regions === 1 ? newInjection : "";
    });
    if (regions !== 1) {
      throw new PatchError(
  ...
  // Markers: 2 (send + prewarm) or 3 when a wham strip block is already present.
  const markerCount = countOccurrences(source, MARKER_TEXT);
  if (countOccurrences(source, ROUTING_FILENAME) !== 4 || (markerCount !== 2 && markerCount !== 3)) {
...
export function patchWhamUpsell(assets) {
  // Best-effort usage-upsell strip (V4). Returns "applied" | "already" | "absent".
  // Voids the fields the upsell/limit banners read from the /wham/usage response:
  // rate_limit_upsell, rate_limit_reached_type, model_picker_upsell, plus
  // rate_limit (limit_reached/allowed gates) and rate_limit_reset_credits.
  // Void the banner inputs, but keep rate_limit.primary_window/secondary_window
  // (the usage-details panel reads those). Only the limit_reached/allowed gates
  // are neutralized, so the banner never classifies as "limit reached".
  const stripFields = (r) =>
    "rate_limit_upsell:void 0,rate_limit_reached_type:void 0,model_picker_upsell:void 0," +
    `rate_limit:${r}.rate_limit?{...${r}.rate_limit,limit_reached:void 0,allowed:void 0}:${r}.rate_limit`;
  const oldStripRe = new RegExp(
    escapeRegExp("/*" + MARKER_TEXT) + "V\\d+\\*/try\\{(\\w+)=\\{\\.\\.\\.\\1,[^{}]*\\}\\}catch\\{\\}", "s"
  );
  ...
  for (const filePath of walkAssets(assets)) {
    ...
    const buildStrip = (resp, client, url, brand) =>
      `let ${resp}=await ${client}.safeGet(\`${url}\`,{additionalHeaders:{"OAI-App-Brand":${brand}.toLowerCase()}});` +
      `/*${WHAM_CAPABILITY_TOKEN}*/` +
      `try{${resp}={...${resp},${stripFields(resp)}}}catch{};` +
      "let ";

    // Case 1: older strip block present -> replace it with the current template.
    const oldMatch = oldStripRe.exec(source);
    if (oldMatch !== null) {
      const markerCount = countOccurrences(source, MARKER_TEXT);
      const patched = source.replace(oldStripRe, (_match, respVar) => {
        return `/*${WHAM_CAPABILITY_TOKEN}*/try{${respVar}={...${respVar},${stripFields(respVar)}}}catch{}`;
      });
      if (countOccurrences(patched, MARKER_TEXT) !== markerCount) {
        throw new PatchError("Wham upsell strip upgrade validation failed");
      }
      fs.writeFileSync(filePath, patched, "utf8");
      applied += 1;
      continue;
    ...
    // Case 2: fresh safeGet site -> inject the strip.
    ...
    let count = 0;
    const patched = source.replace(WHAM_RE, (match, resp, client, url, brand) => {
      count += 1;
      if (count !== 1) return match;
      return buildStrip(resp, client, url, brand);
    });
    if (count !== 1) {
...
export function reportPicker(result) {
  if (result === "applied") status("OK", "Picker redirect applied: cross-provider picks fork into the picked model (V7).");
  else if (result === "already") status("OK", "Picker redirect already present.");
  else status("WARN", "Picker redirect site not found; same-provider routing unaffected.");
}

function reportWham(result) {
  if (result === "applied") status("OK", "Usage upsell banner suppression applied.");
...
export function patchPickerRedirect(filePath) {
  // Best-effort UI-layer patch: redirect cross-provider model picks on the
  // follow-up submit path into the app's native fork flow. Fail-open: any
  // mismatch skips this site; the picker keeps working stock.
  const source = fs.readFileSync(filePath, "utf8");
  if (source.includes(PICKER_MARKER)) return "already";
  const matches = [...source.matchAll(PICKER_RE)];
  if (matches.length === 0) return "absent";
  if (matches.length > 1) {
    throw new PatchError(
      `${path.basename(filePath)} picker call pattern matched ${matches.length} times.`
    );
  }
  const m = matches[0];
  const injection = buildPickerInjection(m[1], m[2], m[3], m[4], m[5], m[6]);
  const patched = source.replace(PICKER_RE, () => injection);
  fs.writeFileSync(filePath, patched, "utf8");
  return "applied";
}
...
export async function patchApp(app, providerConfig, backupDir, dryRun) {
  ...
  ensureLiveConfig(providerConfig);
  ...
  try {
    ...
    if (patchedBundle !== null) {
      ...
      if (previous.includes(CAPABILITY_TOKEN) && previous.includes(WHAM_CAPABILITY_TOKEN) && previous.includes(PICKER_MARKER)) {
        status("OK",
          "Request-layer provider routing (start/fork routing + usage-banner suppression) is already installed.");
        return null;
      }
      ...
      reportWham(patchWhamUpsell(assets));
      syntaxCheckBundle(filePath);
      reportPicker(patchPickerRedirect(filePath));
      syntaxCheckBundle(filePath);
    }
